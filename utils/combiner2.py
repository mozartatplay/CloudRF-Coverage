...
def combine_tiff(tiffFiles, tiffCombineName):
	#print (tiffPath, tiffCombineName)
	# Unzip files to extract directory

	tiffFileList = " ".join(tiffFiles)
	if len(tiffFiles) > 0: 
		tempcombine = os.path.splitext(tiffCombineName)[0] + '_temp.tiff'
		print ('Running combiner on on files list => ', tempcombine)
		pstr = 'gdal_merge.py -co compress=LZW -o ' + tempcombine + ' ' + tiffFileList
		...
		print ('No files sent to combiner')
		print ()
...
if not args.split:
	...
	search_path = path_calc  +  '*.tiff'
	fcheck =  glob.glob(search_path)
	#print(fcheck)
	ffilter = [f for f in fcheck if not (re.match(r'.*\_[0-9].tiff', f) or re.match(r'.*\_temp.tiff', f)) ]
	
	numFiles = len(ffilter)
	numBlock = int( numFiles / numSplits)
	pos = 0
	i = 0
	endBlock = False

	print (ffilter, numFiles, numBlock)
	
	while not endBlock:

		fileGroup = ffilter[pos:pos+numBlock]		
		...
		combine_tiff(fileGroup, tiffCombineName)
