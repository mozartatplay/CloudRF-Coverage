...
if len(calcs) == 0:
    ...
    for c in calcs:
        fname = c['fname']
        print ('Fetching filename: ',fname)
        furl = "https://cloudrf.com/API/archive/data.php?"
        furl = furl + "file=" + fname
        furl = furl + "&uid=28915&fmt=tiff&key=3ac099165b846b48ecb8e59d1835eee2eeb3f33c"
        print ('Full URL: ',furl)
        r = requests.get(furl)
        split_fname = fname.split('_')
        postfix = split_fname[len(split_fname)-1]
        fn=path_calc+os.sep+postfix+".tiff"
        filename = open(fn,"wb")
